...
export default {
  ...
  computed: {
    ...
    // 监听cartlist变化 更新当前商铺的购物车信息shopcart同时返回一个新的对象
    shopCart: function () {
      return Object.assign({}, this.cartList[this.shopId])
    },
    // shopCart变化时侯重新计算当前商品的数量
    foodNum: function () {
  ...
  methods: {
    ...
    removeOutCart (categoryId, itemId, foodId, name, price, specs, packingFee, skuId, stock) {
      if (this.foodNum > 0) {
        this.REDUCE_CART({ shopId: this.shopId, categoryId, itemId, foodId, name, price, specs, packingFee, skuId, stock })
      }
    },
    // 加入购物车，计算按钮位置
